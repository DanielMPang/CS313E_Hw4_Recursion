...
def group_sum_5(start, nums, target):
    ...
    if start >= len(nums):
        return False
    if nums[start]%5 == 0:
        ntarget = target - nums[start]
        if start+1 < len(nums) and nums[start+1] == 1:
            return group_sum_5(start+2, nums, ntarget)
        return group_sum_5(start+1, nums, ntarget)
...
def group_sum_clump(start, nums, target):
    ...
    while i < len(nums) and nums[i] == nums[start]:
        i += 1
    count = i-start
    if group_sum_clump(i, nums, target):
        return True
    return group_sum_clump(i, nums, target-nums[start]*count)
...
def split_odd_10(nums):
    ...
    def helper(i, sum1, sum2):
        if i == len(nums):
            if sum1 % 2 == 1 and sum2 % 10 == 0:
                return True
            if sum2 % 2 == 1 and sum1 % 10 == 0:
                return True
            return False
        if helper(i+1, sum1+nums[i], sum2):
            return True
